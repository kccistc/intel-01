...
from queue import Empty, Queue  # Empty 를 사용해야 한다는 힌트였음
...
def thread_cam1(q):
    ...
    core = Core()
    model = core.read_model("./cap-classification/outputs/20230810_211027_export/openvino/openvino.xml")
    one_flag = True
    ...
    while not FORCE_STOP:
        ...
        # abnormal detect, Inference OpenVINO
        input_tensor = np.expand_dims(detected, 0)

        if one_flag is True:
            ppp = PrePostProcessor(model)
            ppp.input().tensor().set_shape(input_tensor.shape).set_element_type(Type.u8).set_layout(Layout('NHWC'))
